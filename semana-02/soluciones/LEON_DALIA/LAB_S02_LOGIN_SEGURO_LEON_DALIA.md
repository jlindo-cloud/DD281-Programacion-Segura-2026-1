...
**Grupo / Sección:** GRUPO N° 3 - 8VO CICLO
...
a) ¿Qué diferencia de seguridad hay entre los permisos `chmod 600` y `chmod 644` para `server.key`?
   ...
   chmod 600 (rw-------): Solo el propietario puede leer y escribir.
   chmod 644 (rw-r--r--): Todos los usuarios pueden leer el archivo

b) ¿Por qué el campo `Common Name` del certificado debe ser `localhost` en este laboratorio? ¿Qué pasaría si pones otro valor?

   El navegador verifica que el Common Name (CN) del certificado coincida exactamente con el dominio al que te conectas en la barra de direcciones.

Si accedes a https://localhost:8443, el CN debe ser localhost

Si accedes a https://127.0.0.1:8443, el CN debe coincidir con la IP o estar en SANSi accedes a https://localhost:8443, el CN debe ser localhost

Si accedes a https://127.0.0.1:8443, el CN debe coincidir con la IP o estar en SAN

Si pones otro valor:

El navegador mostrará un error más severo: "NET::ERR_CERT_COMMON_NAME_INVALID"
...
c) El archivo `server.csr` ya no es necesario después de generar el certificado. ¿Qué harías con él en un entorno de producción?
   ...
   El CSR (Certificate Signing Request) es un archivo temporal que contiene:
     Tu clave pública
...
1.63 segundos aproximadamente

**Pregunta:** ¿Por qué tarda ese tiempo? ¿Es un defecto o una característica de seguridad?

Es una CARACTERÍSTICA de seguridad, no un defecto. El tiempo se debe al factor de coste (rounds=12) de bcrypt, que realiza 2^12 = 4,096 iteraciones de hash deliberadamente costosas computacionalmente.
...
**Resultado obtenido:** _____________________no pude acceder______________________________
...
**Resultado obtenido:** _❌ "Error - Credenciales incorrectas"
**¿El mensaje revela si el usuario existe?** ☐ Sí x No

**¿Por qué es importante que NO revele esta información?**

Para prevenir enumeración de usuarios. Si un atacante puede determinar qué usuarios existen en el sistema (por ejemplo, viendo mensajes como "Usuario no existe" vs "Contraseña incorrecta"),
...
El uso de prepared statements (consultas parametrizadas) con marcadores ?. En lugar de concatenar directamente el input del usuario en la cadena SQL, se envía la consulta con un placeholder y los valores se pasan por separado.
---
...
|---|---|---|
| 1 | `mal1` | Credenciales incorrectas. Quedan 4 intentos|
| 2 | `mal2` |Credenciales incorrectas. Quedan 3 intentos |
| 3 | `mal3` |Credenciales incorrectas. Quedan 2 intentos |
| 4 | `mal4` | Credenciales incorrectas. Quedan 1 intentos|
| 5 | `mal5` | Cuenta bloqueada por 5 intentos|
...
**Resultado obtenido:** no se ejecuta
**¿Qué función del código previene el XSS?**
La función html.escape() que convierte caracteres especiales HTML en entidades segura
...
AUTH_FAILURE - Intento fallido

CUENTA_BLOQUEADA - Bloqueo por múltiples fallos

**¿Qué información registra un evento de fallo?**

Usuario intentado, número de intento actual / máximo, IP del cliente, timestamp exacto.

**¿Por qué es importante registrar los intentos fallidos?**

_Detectar ataques de fuerza bruta en tiempo real

Realizar auditorías forenses después de un incidente

Identificar patrones maliciosos (mismos IP atacando múltiples usuarios)

Cumplir con requisitos regulatorios (GDPR, PCI-DSS, ISO 27001)

Generar alertas automáticas para el equipo de seguridad
---
...
|---|---|---|
| `Strict-Transport-Security` | x Sí ☐ No |max-age=31536000 |
| `X-Content-Type-Options` | x Sí ☐ No |nosniff |
| `X-Frame-Options` | x Sí ☐ No | DENY|
| `Cache-Control` | x Sí ☐ No |no-store |

**¿Para qué sirve el header `X-Frame-Options: DENY`?**

_Previene ataques de Clickjacking (secuestro de clics). Este header evita que la página sea incrustada dentro de un <iframe>, <frame>, <embed> o <object> de otro sitio web. Un atacante no puede crear una página maliciosa que cargue tu sitio en un iframe transparente para engañar al usuario y hacer que haga clic en elementos sin su conocimiento
...
|---|---|---|
| **Protección SQL Injection** |❌ Concatenación directa de strings | ✅ Prepared statements con ?|
| **Almacenamiento de contraseñas** |❌ Comparación directa (texto plano conceptual) |✅ bcrypt con salt y factor 12 |
| **Verificación de método HTTP** |❌ Acepta GET y POST | ✅ Solo acepta POST|
| **Protección XSS en output** |❌ Sin escape de caracteres | ✅ html.escape()|
| **Manejo de intentos fallidos** |❌ Sin límite | ✅ Bloqueo tras 5 fallos por 15 min|
| **Logging de eventos** |❌ Ninguno |✅ INFO, WARNING |
| **Mensajes de error** |❌ Revelan si usuario existe |✅ Genéricos (no enumeran usuarios) |
| **Headers de seguridad HTTP** | ❌ Ninguno| ✅ HSTS, X-Frame-Options, etc.|
| **Timing attack prevention** |❌ No |✅ Hash dummy + sleep(0.5) |
...
bcrypt produce una salida de longitud fija (60 caracteres) independientemente de la longitud de la contraseña de entrada. Esto es una característica de diseño porque:

El hash es el resultado de una función matemática que siempre produce el mismo tamaño

Oculta información sobre la longitud de la contraseña original

Facilita el almacenamiento en la base de datos

**¿Podrías saber cuál usuario tiene la contraseña "más simple" solo mirando los hashes?** ☐ Sí x No — **¿Por qué?**

_bcrypt es una función hash criptográficamente segura y unidireccional que produce una salida aparentemente aleatoria. Contraseñas "simples" como "123456" y contraseñas "complejas" como "Kj9#2mQ$xL!pR" producen hashes de igual longitud y aspecto similar. No hay correlación visible entre la "simplicidad" de una contraseña y su hash. La única forma de verificar una contraseña es probarla directamente con bcrypt.checkpw().
...
_Mejora 1: Autenticación de dos factores (2FA)
Implementar TOTP (Time-based One-Time Password) usando estándar RFC 6238 con Google Authenticator o similar. El usuario debe ingresar un código de 6 dígitos que cambia cada 30 segundos, además de su contraseña. Esto mitiga el riesgo de contraseñas robadas o filtradas.

Mejora 2: Sesiones JWT seguras con refresh tokens
Implementar un sistema completo de sesiones usando JWT (JSON Web Tokens) con:

Tokens de acceso de corta duración (15 minutos)

Refresh tokens rotativos almacenados en httpOnly cookies

Blacklist para revocación de tokens

Protección contra token replay

Mejora 3: Rate limiting exponencial y captcha
Implementar un mecanismo de protección contra fuerza bruta más robusto:

Exponential backoff: tiempos de espera que aumentan exponencialmente (1s, 2s, 4s, 8s...)

Captcha (hCaptcha o reCAPTCHA) después de 3 intentos fallidos

Bloqueo por IP después de múltiples fallos desde la misma dirección
