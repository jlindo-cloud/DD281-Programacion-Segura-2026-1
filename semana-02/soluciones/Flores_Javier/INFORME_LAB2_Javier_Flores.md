...
a) ¿Qué diferencia de seguridad hay entre los permisos `chmod 600` y `chmod 644` para `server.key`?
   ...
   *Tu respuesta:* El 600 es la regla de oro: solo tú puedes leer tu clave privada, lo cual es vital para que nadie más husmee. Si usas 644, básicamente dejas la puerta abierta a que cualquier usuario del sistema pueda copiar tu clave y suplantar tu servidor. En seguridad, siempre apuesta por el 600.

b) ¿Por qué el campo `Common Name` del certificado debe ser `localhost` en este laboratorio? ¿Qué pasaría si pones otro valor?

   *Tu respuesta:* Porque tu navegador espera que el certificado coincida exactamente con la dirección a la que te conectas. Si escribes localhost en el navegador pero el certificado dice "misitio.com", el navegador detecta que no hay coincidencia y te bloquea con un error de seguridad. Es una medida para evitar que alguien se haga pasar por otro sitio.
...
c) El archivo `server.csr` ya no es necesario después de generar el certificado. ¿Qué harías con él en un entorno de producción?
   ...
   *Tu respuesta:* Una vez que ya tienes tu certificado firmado, el .csr es como un boceto viejo: ya no sirve para nada operativo. Lo mejor es borrarlo de forma segura del servidor. No tiene sentido dejar archivos basura que solo ocupan espacio y que, si alguien entrara al sistema, podrían darle pistas sobre cómo está configurada tu seguridad.
...
*Tu respuesta:* 1 segundos aproximadamente

**Pregunta:** ¿Por qué tarda ese tiempo? ¿Es un defecto o una característica de seguridad?

*Tu respuesta:* Por que se ejecuto desde un script en python, es una caracteristica de seguridad
...
Por que el certificado SSL le falta de una "Autoridad de Certificación" (CA) confiable y El "Nombre de host" no coincide (Common Name Mismatch)
...
**Resultado obtenido:** Contraseña Incorrecta

**¿El mensaje revela si el usuario existe?** ☐X Sí ☐ No
...
|---|---|---|
| `' OR '1'='1` | Acceso denegado | Acceso denegado|
| `admin' --` | Acceso denegado |Acceso denegado|
| `'; DROP TABLE usuarios; --` | Acceso denegado | Acceso denegado|

**¿El sistema fue vulnerable?** ☐ Sí ☐X No
...
|---|---|---|
| 1 | `mal1` | Contraseña Incorrecta |
| 2 | `mal2` | Contraseña Incorrecta|
| 3 | `mal3` | Contraseña Incorrecta|
| 4 | `mal4` | Contraseña Incorrecta|
| 5 | `mal5` | Contraseña Incorrecta|
| 6 (con clave correcta) | `SecurePass456#` | Cuenta bloqueada|

**¿Cuál es el tiempo de bloqueo configurado en el sistema?** 15 minutos
...
La función principal que previene ataques de XSS (Cross-Site Scripting) en el código es html.escape().
...
Evento de info y warning

**¿Qué información registra un evento de fallo?**

fecha, hora, tipo de evento, usuario, tipo de acceso
**¿Por qué es importante registrar los intentos fallidos?**

los logs no solo sirven para vigilar, sino que son el detonante necesario para que tus funciones de seguridad (como el bloqueo temporal) entren en acción.
...
|---|---|---|
| `Strict-Transport-Security` | ☐ Sí ☐X No | |
| `X-Content-Type-Options` | ☐ Sí ☐X No | |
| `X-Frame-Options` | ☐ Sí ☐X No | |
| `Cache-Control` | X Sí ☐ No |max-age=0 |

**¿Para qué sirve el header `X-Frame-Options: DENY`?**
Sirve para evitar que tu sitio web sea incrustado dentro de un iframe, frame o object en otras páginas.
Es una medida de seguridad fundamental contra un ataque llamado Clickjacking (secuestro de clics).
...
|---|---|---|
| **Protección SQL Injection** |Vulnerable (concatenación) |Seguro (consultas parametrizadas) |
| **Almacenamiento de contraseñas** |Texto plano (vulnerable) |Cifrado (bcrypt con salt) |
| **Verificación de método HTTP** |No validado |Validado (solo permite POST) |
| **Protección XSS en output** |Sin protección |Escapado (html.escape) |
| **Manejo de intentos fallidos** |Ninguno (ataque libre) |Bloqueo (tras 5 intentos, 15 min) |
| **Logging de eventos** |Inexistente |Auditable (auth.log) |
| **Mensajes de error** |Informativos (revelan usuario) |Genéricos (evitan enumeración) |
| **Headers de seguridad HTTP** |Ninguno |Implementados (HSTS, X-Frame, etc.) |
| **Timing attack prevention** |Vulnerable (comparación directa) |Seguro (comparación de bcrypt) |
...
**¿Todos los hashes tienen el mismo prefijo `$2b$12$`?** ☐X Sí ☐ No

**¿Por qué todos los hashes tienen la MISMA longitud aunque las contraseñas sean distintas?**
La razón por la que todos los hashes tienen la misma longitud (independientemente de si la contraseña es "123" o una frase de 50 caracteres) se debe a que las funciones de hash, como bcrypt, están diseñadas bajo el principio de longitud de salida fija.
_________________________________________________________________________

**¿Podrías saber cuál usuario tiene la contraseña "más simple" solo mirando los hashes?** ☐ Sí ☐X No — **¿Por qué?**

No,por diseño, es imposible saber cuál usuario tiene la contraseña más "simple" con solo mirar los hashes en la base de datos.
_________________________________________________________________________
...
1. Generación: En el servidor, se crea un token único y aleatorio (usando secrets.token_hex) cada vez que un usuario carga la página de inicio de sesión.

2. Inyección: Este token se inserta en un campo <input type="hidden" name="csrf_token" value="..."> dentro del formulario HTML. Así, el token viaja junto con los datos de usuario y contraseña cuando se hace clic en "Ingresar".

3. Validación: Al recibir el POST en login_seguro.py, el servidor extrae el token enviado por el formulario y lo compara con el valor que el servidor generó previamente para esa sesión.

4. Bloqueo: Si los tokens no coinciden, o si el formulario no trae el token, el servidor rechaza la petición inmediatamente. Un atacante no puede robar este token porque está protegido por la política de seguridad del navegador (Same-Origin Policy), lo que impide que sitios externos lean el contenido de tu página.
...
He transformado un script de autenticación vulnerable en un sistema robusto mediante la implementación de hashes con bcrypt, consultas SQL parametrizadas y protección contra ataques CSRF y XSS. Aprendí que la seguridad no depende de una única medida, sino de una arquitectura integral donde la validación de entradas, el manejo seguro de sesiones y la configuración de cabeceras HTTP son vitales para proteger la integridad del usuario. Además, integré un sistema de auditoría centralizado que permite registrar eventos críticos, facilitando la detección proactiva de intentos de acceso no autorizados. Esta experiencia me permitió comprender cómo un atacante puede explotar vulnerabilidades básicas y cómo aplicar contramedidas técnicas para neutralizarlas eficazmente. Con esta base, mi laboratorio ahora cuenta con una arquitectura de defensa en profundidad lista para entornos de producción.
...
8. **Reflexión final**: ¿Qué cambiarías en el login seguro para hacerlo más robusto? Menciona al menos 3 mejoras posibles.
   ...
   1. 2FA (Autenticación de dos factores): Añade una capa extra de seguridad solicitando un código temporal desde el móvil para acceder.

   2. Sesiones del lado del servidor: Sustituye la validación constante de credenciales por un token de sesión seguro, efímero y almacenado en el servidor.

   3. Rate Limiting por IP: Bloquea direcciones IP sospechosas tras varios intentos fallidos para detener ataques de fuerza bruta distribuidos.
